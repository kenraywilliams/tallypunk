...
- **Stakeholder status (STK-06)**: Stakeholders list **Status** column + Profile "Vesting status" row — Active/Paused pill/Terminated pill by vesting reality; person-level event line on Profile; **creating a grant for a terminated person shows the gold "status will show Active again" notice**; a pause whose end passed reads Active.
