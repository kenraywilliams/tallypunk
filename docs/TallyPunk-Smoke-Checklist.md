...
- **Bulk actions v1 (GBL-10)**: **Bulk actions** button next to Columns ▾ on both lists → checkboxes + header select-all (**filtered rows only**); tighten a filter with rows selected → hidden rows drop out of the selection and the count updates; Terminate…/Pause… open **itemised previews** — the confirm inside is the only trigger; **grants-list bulk UPDATES existing events**: re-pausing a paused grant replaces the window (preview: "current pause … — will be overwritten"); re-terminating moves the date **either direction** — later moves are pool-pre-flighted cumulatively (short pool → red note + inline "+ Add N" / "Make ∞" remedies; only blocked moves skip); **after a batch the selection clears and acted rows flash**; stakeholder-level bulk still skips already-evented people and the **toast counts the skips**; bulk bar shows a "select the objects…" hint before the first tick (no layout jump); fully-vested-then-terminated grants' banner says "nothing was forfeited"; row click toggles selection in bulk mode; Done/Cancel exits and clears; live summary rows open the person/grant.
...
- **Pool delete wizard**: pool view → **Delete…** → 3 choices (General Pool default / choose pool / delete all grants) + per-grant overrides; a finite target short on room shows the red shortfall with **"+ Add N" / "Make it Infinity ∞"** inline; typed "delete"; summary grouped by stakeholder. **General Pool**: NOT a row in the Pools list — a muted "holds N granted units — open" line under the table opens its view (no Edit/Delete, "inherent structure" note); still selectable in the grant pool picker and listed in the Grants-list pool filter.
...
- **Grants vested columns**: fresh view shows **Vested %** by default; **Vested #** (units) and **Vested** (combined % · #) live in the Columns menu; saved views that had the combined column migrate to Vested %; all three sort numerically.
